...
/**
 * Create a quarterly note for the given date using Calendar-owned settings,
...
export async function tryToCreateQuarterlyNote(
  ...
  const quarterlySettings = settings.quarterly;
  const filename = date.format(quarterlySettings.format);
  const path = getPeriodicNotePath(quarterlySettings, date);

  const createFile = async () => {
    await ensureParentFolderExists(path);
    const { contents: rawTemplate, foldInfo } = await getTemplateInfo(
      quarterlySettings.template
    );
    const expanded = applyTemplateTokens(
      rawTemplate,
      "quarterly",
      date,
      quarterlySettings.format
    );
